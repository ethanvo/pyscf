...
def update_t2(cc, t1, t2, eris):
    ...
    Fvv = imdk.cc_Fvv(t1, t2, eris, kconserv)
    Fov = imdk.cc_Fov(t1, t2, eris, kconserv)
    Loo = imdk.Loo2(t1, t2, eris, kconserv)
    Lvv = imdk.Lvv2(t1, t2, eris, kconserv)
    ...
    if (nocc ** 4 * nkpts ** 3) * 16 / 1e6 + mem_now < cc.max_memory * .9:
        Woooo = imdk.cc_Woooo2(t1, t2, eris, kconserv)
    else:
        fimd = lib.H5TmpFile()
        Woooo = fimd.create_dataset('oooo', (nkpts, nkpts, nkpts, nocc, nocc, nocc, nocc), t1.dtype.char)
        Woooo = imdk.cc_Woooo2(t1, t2, eris, kconserv, Woooo)
    ...
    time0 = log.timer_debug1('update t2', *time0)
...
class KRCC2(pyscf.cc.ccsd.CCSD):
